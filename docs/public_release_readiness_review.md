...
> **닫혔다 (2026-08-10 · R3).** 계획서
> [`plan_r3_r6_public_polish.md`](_private/project/plan_r3_r6_public_polish.md).
> **범위가 이 절이 적은 TTL 한 줄이 아니라 추적 파일 12곳이었다** — `CITATION.cff` ·
> BibTeX 3곳 · **GitHub Pages 라이브 데모 URL 7곳** · 생성기 `build_owl.py`·`build_viz.py`.
> 리포명은 `config/namespaces.py` 의 `REPO_SLUG` **한 곳**에서만 정하고, 발행되는 URL 은
> 거기서 조립한다. 대상은 `arkwith7/sdkb-dataset` 으로 확정(사용자 결정 2026-08-10).
>
> `seeAlso` 는 **앵커가 아니라 파일**을 가리킨다 — 제목이 바뀌면 앵커는 조용히 깨지지만
> 파일 경로는 게이트가 잡을 수 있다. 아키텍처 문서는 F2 에서 공개 유지로 판정됐으므로
> **둘 다 발행한다**(1차 `ontology_guide.md` · 2차 아키텍처 문서).
>
> 회귀는 검사기가 막는다 — 공개 트리에 옛 슬러그가 남으면 실패한다. **첫 실행에서
> 검사기가 스스로 한 건을 잡아냈다**: `config/namespaces.py`, 즉 그 문자열을 *정의하는*
> 파일. 인용 허용 목록은 코드에 두고 사유를 적는다(이 문서 + 그 정의 파일 둘) — 파일이
> 스스로 면제를 선언하게 하면 그것은 검사가 아니라 우회로가 된다.
...
> **닫혔다 (2026-08-10 · R4 — F5 와 함께).** 아래 F5 의 갱신 블록을 보라. 요지는 하나다 —
> **README 의 수치 자리에서 손으로 적은 숫자를 없앴다.** 고친 것이 아니라 없앴다.
...
> **닫혔다 (2026-08-10 · R4).** 신규 `scripts/report_graph_signature.py` + `make signature` ·
> `signature-inject` · `signature-check`. **CLAUDE.md §4 가 요구하는 그래프 서명을 이행하는
> 코드가 그때까지 없었다** — 그것이 F4·F5 의 근인이다.
>
> | 어긋났던 것 | 조치 |
> |---|---|
> | `README:109` `# 14 core classes` (실측 43) | 트리 주석에서 **숫자를 뺐다** — 서명 블록을 가리킨다 |
> | `README:102` `14-type process KG` | 타입 열거로 바꿨다 |
> | `README:117,252` 큐레이션 `229/268` (실측 274/312) | 서명 블록이 생성한다 |
> | `README:27` 라이브 데모 설명의 `229 nodes` | 숫자를 뺐다 |
> | `README:259` `75 passed / 10 skipped · OWL 438 triples` | **덤으로 발견** — 낡았다. `make test` 를 가리키게 바꿨다 |
>
> **고치는 대신 없앴다.** 손으로 적은 숫자를 정확한 숫자로 바꾸면 다음 원천 변경에서 또
> 어긋난다. 그래서 그 자리에는 숫자를 두지 않고, 정본 수치는 마커
> (`<!-- sdkb:signature:begin -->`) 안에서 **코드가 쓴다.** `tests/test_graph_signature.py`
> 가 블록이 낡으면 실패시킨다 — 사람이 `make signature` 를 잊는 것이 정상이기 때문이다.
>
> **§2.1 의 어긋남도 이때 풀렸다.** G1 이 적은 *"ObjectProperty 93"* 은 다른 집계가 아니라
> [`ontology_guide.md`](ontology_guide.md) §2.1 표의 **총계 행이 틀렸던 것**이다 — 모듈별
> 열은 처음부터 99 로 합산된다. 아무도 다시 더해 보지 않았고, 그것이 수치를 생성해야 하는
> 이유 그 자체다. G1 의 그 줄은 소급 수정하지 않는다(그때의 판단 기록이다).
...
> **닫혔다 (2026-08-10 · R6).** 5문서 전부에 머리 영문 요약(10–14줄)을 넣었다. 열 이름
> 병기는 **언어와 무관한 사실을 담은 표에만** 걸었다 — 스키마 문서의 필드·경로 표, 데이터셋
> 카드의 항목·축 표, 비식별 프로토콜의 층 표(6개 헤더).
> **runbook 의 운영 표는 한국어로 남겼다** — 이 절이 적은 그대로, KIPRIS/KIPO 화면과 양식의
> 이름이 한국어여서 옮기면 절차를 따라가기가 더 어려워진다. 요약에 그 판단을 명시했다.
> 전량 번역은 하지 않았다.
...
> **닫혔다 (2026-08-10 · R5) — 그리고 이 절의 요약 문장은 틀렸다.**
> 결손은 정렬 3모듈 24곳이 아니라 **42곳**이고, **가장 큰 덩어리가 특허 모듈(18곳)** 이었다.
> 위 표에 그 숫자가 실려 있는데(OP 23/32 · DP 17/26) 본문이 *"특허는 문서화가 잘 돼 있다"*
> 로 반대로 요약했다. 표는 소급 수정하지 않는다 — 그때의 측정이고, 요약이 그것을 잘못 읽었다.
>
> 특허 모듈이 특히 무거웠던 이유는 하류가 그 술어를 직접 쓴다는 것이다 —
> `filingDate`·`publicationDate`·`rejectionDate` 가 주석 없이 나란히 있었고, CLAUDE.md §5-4
> 가 *"출원일·공개일·등록일은 서로 다르며, 섞이면 시계열 연구가 통째로 무효가 된다"* 고
> 경고한 자리가 정확히 여기다.
>
> **보강 후 268/268**(주석 트리플 +42 · 클래스 84 · OP 99 · DP 85 **불변** · domain/range
> **불변**). 방향이 모호한 술어는 어느 쪽이 주어인지 문장에 적었고(`ont:barrierOf` = barrier →
> segment · `ont:scenarioDriver` = scenario → factor), **domain 이나 range 를 일부러 두지 않은
> 둘**(`ont:hasPriorArtApplicant` · `ont:fundedBy`)은 그 이유를 주석에 적었다 — 비어 있는 것과
> 뜻이 있어 비운 것은 다르다. `tests/test_graph_signature.py` 가 268/268 을 고정한다.
...
| ~~R3~~ | ~~`sdkb-core.ttl` 의 `seeAlso` URL 교체 (F3)~~ | **완료 (2026-08-10)** — 범위는 TTL 한 줄이 아니라 추적 파일 **12곳**이었다. 리포명은 `config/namespaces.py` 한 곳에서 정한다 |
| ~~R4~~ | ~~README 클래스 수·그래프 규모 정정 + 서명 자동 산출 (F4·F5)~~ | **완료 (2026-08-10)** — `make signature` 신설. 수치를 고친 것이 아니라 **손으로 적는 자리를 없앴다** |
| ~~R5~~ | ~~정렬 3모듈 술어에 `rdfs:comment` 보강 (G5)~~ | **완료 (2026-08-10)** — 결손은 24곳이 아니라 **42곳**이었고 최대 덩어리는 특허 모듈이었다. 268/268 |
| ~~R6~~ | ~~공개 대상 문서 영문 요약 (F6)~~ | **완료 (2026-08-10)** — 5문서 머리 요약 + 언어무관 표 6개 헤더 병기. 전량 번역은 하지 않았다 |

**권고 순서: ~~R1~~ → ~~R3~~ → ~~R4~~ → ~~R6~~ → ~~R5~~.** **R1–R6 전부 완료 (2026-08-10).**
R3–R6 은 사용자 승인(2026-08-10) 아래 한 요구정의로 묶여 §2 절차를 탔다 —
[`plan_r3_r6_public_polish.md`](_private/project/plan_r3_r6_public_polish.md).

**남은 것은 이 문서의 소관이 아니다.** 리포를 실제로 공개하는 일(새 리포 orphan 커밋 ·
릴리스 태그 · DOI)과 KIPRIS 키로 돌려야 닫히는 CR-015 ③ · CR-016 ② · D-38 ③④ 가 남아 있다.
**R5 가 T-Box 파일 다섯의 sha256 을 움직였으므로**(의미 델타 0 · 주석 델타 42) 원고가 인용할
태그는 원고가 실제로 쓴 커밋에 따로 달아야 한다 — 계획서 §6 참조.
