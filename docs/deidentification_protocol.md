...
움직이지 않았다** → 하류 H1·H2·RQ3 결론 불변([[sdkb-is-upstream-baseline-for-h1]]).
...
하류 `sdkb-foresight-paper` 가 **특허 초록에서 결함모드를 추출하는 층**을 신설했다(커밋 `dfdb9dd`,
§G1 Phase C). 이름이 "문제"라 위 §2 의 기밀 `ont:Problem`(226) 축과 혼동되기 쉬우나 **원천과
...
2026-07-22 신설 **CQ28** 특허↔문제↔전문가 다리 — G₀ 에서 40행 응답), 하류
