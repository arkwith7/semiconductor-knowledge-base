...
동결된 조작적 정의 (결과를 보기 전에 · 계획 파일 2026-09-08 · 사용자 승인):
  ...
  **경로(path)** 와 **유량(flow)** 을 구분한다. 경로 = 그 공리를 읽는 소비자가 존재한다.
  ...
  경로가 있고(생성기가 읽었다) 유량이 0 이었다(채굴 쌍이 없다) — 그 둘을 한 열에 뭉개면
  "삭제"와 "대기"가 갈리지 않는다. (그 둘은 7-0 일몰 실행으로 지워졌다 — 표에는 기록으로 남는다.)
...
#: 7-0(2026-09-09 · D-S) 이 일몰 조항의 `substitutableWith` 2건을 지웠다 — 역시 기록으로 남긴다.
#: tests/test_stage6_ablation.py 가 열거 ⊆ 표, 표 ∖ 열거 == 6-B 삭제분 ∪ 7-0 삭제분 을 고정한다.
_KEEP_LEGACY = ("보존 (§7-2 · 하류 핀)", "단계 1 재현 · legacy T-Box 는 제자리 불변 · sdkb-patent.ttl 0a317389… 핀")
_SUNSET = ("삭제 (7-0 · 일몰 조항 실행)", "경로 有(생성기가 읽었음) · 유량 0 — PLAN-002 채굴 쌍 없음 · 단계 7 착수 시점에 Prec 미착수 → D2 대로 삭제")
