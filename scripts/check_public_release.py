...
LEGACY_SLUG_ALLOWED = {
    ...
    # 하네스는 **스냅샷 출처를 기록**하므로 상류 슬러그를 적는다. 소스를 고쳐 지우면
    # "복사하되 고치지 않는다"가 깨지고, 공개본이 감사 대상 소스가 아니게 된다.
    "benchmark/src/sdkb_paper/config.py",
    "benchmark/src/sdkb_paper/ontology/vendor.py",
}
