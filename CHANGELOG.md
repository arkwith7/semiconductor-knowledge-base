...
- **심사관이 직접 짝지은 구성요소 단위 판정 305건을 얻었다.** 의견제출통지서의
  `<표 N>` 구성 대비표에서 (출원, 표, 구성그룹, 구성번호, 캡션청구항, 판정)을 뽑아
  `data/patents/notice_element_judgments.parquet` 에 담았다 — 출원 26 · 구성그룹 67 ·
  판정 `Identical` 188 · `Different` 113 · `Corresponding` 4. 원문 어휘는 전부
  **`실질적 동일`** 이고 단순 `동일` 은 0건이다(`judgment_raw` 로 함께 싣는다).
- **텍스트 열은 담지 않는다.** OCR 이 표의 열 정렬을 무너뜨려 (본원 구성, 인용 개시) 쌍이
  조각으로만 남는다 — 305행 중 **142행의 본문이 4자 미만**이다. 조각을 개시 텍스트로
  승격하면 이름이 의미와 달라진다(§1-3). 담지 않으므로 이 산출물은 원문 발췌를 갖지 않는다.
- **`구성 N-M` 의 앞 숫자는 청구항 번호가 아니다.** 초판 설계는 그것을 `claim_no` 라
  불렀고 **틀렸다** — 표 단위 구성 그룹 번호이며 청구항 번호는 표 캡션에 있다.
  `1020127009380` 의 `<표2>` 는 `구성 2-1…2-6` 을 담는데 전부 **청구항 3** 의 구성이다.
  전수 대조로 일치 258(84.6%) · **불일치 9(3.0%)** · 캡션없음 38(12.5%). `element_group`
  과 `caption_claim_no` 를 **별개 컬럼으로 분리**했고, 캡션이 없으면 비운다(추측하지 않는다).
- **LLM 을 쓰지 않았다** — 통지서에 심사관 실명이 있어 외부 전송이 금지되고(§1-5),
  구조 키는 숫자와 하이픈뿐이라 규칙으로 충분하다. 결정적이다(두 번 빌드 sha256 동일).
- **누출 통제** — 범위는 기존 `harvest_scope_dev_train.json`(dev+train 800출원, test·test_b
  제외)이며 새로 만들지 않았다. 305행은 **전량 범위 안**이라 한 행도 잃지 않는다.
  `check_leakage.py` 는 이 저장소에 **존재하지 않으므로**(PLAN-001 이 이름만 적어 두었다)
  그것이 검증한다고 주장하지 않는다 — 회귀 테스트가 대신 고정한다.
- **발행하지 않는다** (사용자 결정). 2-A 의 `notice_legal_basis.parquet` 과 같은 이유이며
  더 강하게 적용된다 — 구성요소 단위 심사 판단은 세부이고 한 번 발행하면 회수가 어렵다.
  생성기와 집계 리포트는 발행하므로 원문을 가진 사람은 동일 산출을 재생성할 수 있다.

**게이트는 두 번 받았고 그 사이에 계측기를 두 번 고쳤다 — 두 수를 모두 남긴다.**

| | 값 |
|---|---:|
| 1차 사람 대조 (교정 전) | 25/30 = **0.8333** — 미달 |
| 2차 사람 대조 (교정 후, 같은 표본·시드) | 30/30 = **1.0000** — 충족 |

교정 1은 위 `claim_no` 개명이고 **사용자 지적**에서 나왔다. 교정 2는 **대조 시트가 다른
표의 캡션을 보여준 것**이다 — 빈 줄 없는 문서에서 앞 표의 창이 뒤 표의 행을 삼켰다.
**1차의 오답 5건은 전량 이 형태였고, 검토자의 판정이 옳았다** — 다섯 행 모두 원문 대조에서
parquet 값이 맞았다. 판정이 옳았고 계측기가 틀렸다. 파서는 두 교정 모두에서 무사했다
(블록 닫힘 규칙을 바꿔 재계산해도 305행 그대로 · 결과가 다른 문서 0건).

**§0 소비자에게 달라지는 것 — 없다.** TTL · T-Box · shape · IRI · 어휘 무변경이고 그래프
서명도 불변이다. 이 데이터가 그래프에 들어가는 것은 **단계 4** 이며 그때 별도 승인을 받는다.
...
- **`patent-text` 프로파일이 특허 A-Box 생성에 처음으로 쓰인다.** `Bridge` 가 프로파일을
  받을 자리가 없어(`build_lexicon`·`load_aliases` 의 기본값이 그대로 걸렸다) 생성기 여섯이
  전부 `expert-tag` 어휘로 특허 본문을 읽어 왔다. `Bridge(root, morph, profile=…)` 를 열고
  특허 계열 넷과 평가·진단 둘이 `patent-text` 를 **명시**한다.
  **기본값은 `expert-tag` 그대로다** — 뒤집으면 전문가·문제 A-Box 가 조용히 움직인다.
- **발행 사전의 `blocked` 를 A-Box 어휘가 소비한다.** 지금까지 R4·R6·R7 은 사전 층에서만
  작동했고 A-Box 는 KG 에서 직접 어휘를 지어 그 규칙을 보지 못했다. 두 경로를 쌍 단위로
  대조하니 차이가 **정확히 `blocked` 9건**이었고(`hf`·`high k`·`레시피`·`레티클`·`오버레이`·
  `온도`·`응력`·`종료점`·`종말점`), 그것을 소비하도록 배선했다. 이제 **A-Box 어휘 ≡ 발행 사전**
  이며 `tests/test_cr020_profile_wiring.py` 가 그것을 쌍 단위로 단정한다.
  **사전(`concept_mapping.json`)은 한 글자도 바뀌지 않았다.**
- **`expert-tag` 산출물은 바이트 불변.** `sdkb-abox-experts-problems.ttl`
  `4251b65a…` · `abox_linking_report.json` `06e62514…` — 이 변경이 특허 계열만 건드렸다는
  기계적 증명이다.
- **A-Box 개념 링크가 축을 바꾼다(같은 코드에서 프로파일만 교체한 두 팔로 측정).**
  특허 A-Box 33,931 → **34,117** 트리플 · `Skill` 630 → **14** · `EquipmentClass` 7 → **176** ·
  `Material` 520 → **926** · 개념 0개 특허 23 → **19**. 인용문헌 A-Box 57,063 → **57,700** ·
  `Skill` 1,364 → **262** · `EquipmentClass` 9 → **268**. B층 질의 `Skill` 66 → **1** ·
  고아 83 → **74**.
- **한정요소 투영(CR-017)** `mappings/claim_features.parquet` — 행 1,306,191 불변 ·
  **개념 0개 한정요소 71.7 % → 66.9 %**(독립항 73.4 % → **68.4 %**) · 개념 링크 529,151 →
  **615,436** · `skill:` **171,417 → 4,146** · `equipment_class:` 405 → **46,175**.
  **감소는 개념 단위로 전량 귀속된다** — 재지정(`가스`·`챔버`·`마스크`·`플라즈마`·`산화물`·
  `절연막`), `blocked` 소비(R4·R6·R7), 그리고 사전이 `patent-text` 에 애초에 싣지 않은
  ≤4자 한글 표면형(`결함`·`수율`). **설명되지 않는 감소 0.**
- **CR-001B 가 수확한 한국어 표면형이 이제 A-Box 에 도달한다** — `유전체` 72특허 ·
  `절연층` 61 · `성막` 58 · `유량` 38. 어휘를 늘려도 배선이 없으면 A-Box 는 그것을 보지
  못한다는 것이 D-49 의 내용이었다.
- **T-Box·IRI·어휘 불변** · SHACL 전량 conform · pytest 278 → **290 통과**.
- **하류 영향 (§0)**: 스냅샷 서명이 바뀌므로 `sdkb-prior-art-paper` 는 §2.1 경로로 전량
  재측정한다. **CR-001B 성공기준 ⑤ 와 이 CR 의 ⑥ 은 한 스냅샷에 함께 실리므로 하나의
  판정이다.** 이 변경이 검색을 개선한다고 주장하지 않는다 — 자원 지표의 개선이 태스크
  지표로 이어지지 않은 관측이 이미 셋 있다(하류 D-23·D-48).

## [1.1.1] — 2026-08-21 · release-metadata correction

Tagged `v1.1.1-paper` · concept DOI [10.5281/zenodo.22030395](https://doi.org/10.5281/zenodo.22030395).
**Ontology, data, code and evaluation assets are byte-identical to 1.1.0.** Of the 324 published
assets, seven hashes move: the five metadata files listed below, plus `data/reports/cq_report.json`
and `data/reports/public_release_manifest.json`, which are regenerated by the release build and
carry non-deterministic fields (per-query wall-clock seconds and a generation timestamp). The
competency-question outcome is unchanged — 14/31 out of the box, the same failures in the same
suites. Nothing else in the tree differs, and no asset was added or removed.

**Why this release exists.** The tree tagged `v1.1-paper` contradicted itself. Its `README.md`
opened with `# SDKB v1.0`, its BibTeX title said `SDKB v1.0`, and its `CITATION.cff` carried no DOI
at all — while its `.zenodo.json` and `CITATION.cff` version field both said `1.1.0`. The two
commits that fixed the prose and registered the DOIs landed **after** the tag, so they were never
archived. Zenodo archives the tree at tag time, which means anyone resolving the version DOI
downloaded an archive that called itself v1.0. A downstream paper cites that DOI as its
reproduction baseline, so the contradiction was visible to its reviewers.

**What changed, and the rule it establishes.** `CITATION.cff` (version `1.1.1`, date-released
`2026-08-21`, and the hardcoded version-DOI identifier replaced by the tagged-tree URL) ·
`.zenodo.json` (version, related tree URL) · `README.md` and `README.ko.md` (BibTeX version and
the citation guidance) · this file. **A tagged tree never states its own version DOI.** That number
is minted when the release is published, so a tag can only ever name the *previous* one — which is
precisely how `v1.1-paper` came to point at a DOI that was not its own. The concept DOI is stable,
lists every version, and each version names the tag it was cut from; that is the durable path.
