...
> **지위 갱신 2026-09-05 — C-5(작동 증명)의 산출 경로.** 하류 논지 피벗(하류
> `01.code_spec/plans/PLAN-092`)으로 이 실험은 **후속 논문이 아니라 하류 현 논문 §5.3 의 결과**를
> 낸다. E1 `SemanticPathRecall` 이 *"추론으로 생긴 경로가 태스크 응답에 도달하는가"* 를 재며,
> 그것이 조건 C-5 의 정의다. **선행 조건은 그대로다** — PLAN-001 이 승인·구현되지 않으면
> 착수하지 않는다. 순서는 [PLAN-004](PLAN-004-kb-sufficiency-roadmap.md).
>
> **실행과 판정은 하류에서 난다.** 자원이 바뀌면 하류 §2.1(새 사전등록 동결)이 먼저 발동하고,
> E1–E5 는 새 코드이므로 하류 §2 의 정지 게이트 넷을 탄다.
